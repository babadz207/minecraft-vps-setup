...
foreach ($inst in $targetInstances) {
    ...
    if (Test-Path $rpDir) {
        ...
        foreach ($zp in $zipPacks) {
            if ((Test-Path $zp) -and ((Get-Item $zp).Length -gt 10000000)) { $sourceZip = $zp; break }
        }
        if ($sourceZip) {
            foreach ($zp in $zipPacks) {
    ...
    if (Test-Path $instCfgPath) {
        ...
        if ($newCfg -notcontains "JoinServerOnLaunch=true") { $newCfg += "JoinServerOnLaunch=true" }
        if ($newCfg -notcontains "JoinServerOnLaunchAddress=donutsmp.net") { $newCfg += "JoinServerOnLaunchAddress=donutsmp.net" }
