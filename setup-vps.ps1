...
$exactPackName = "beatrix_shop 1.9v1 (1).zip"
$targetPack = Join-Path $ResourcePacksDir $exactPackName
$packReady = $false
...
$allVariations = @(
    ...
    (Join-Path $ResourcePacksDir "beatrix_shop.zip")
)
foreach ($cand in $allVariations) {
    if ((Test-Path $cand) -and ((Get-Item $cand).Length -gt 10000000)) {
        if ($cand -ne $targetPack) { Copy-Item -Path $cand -Destination $targetPack -Force }
        $packReady = $true
        Write-Success "Reused existing $([System.IO.Path]::GetFileName($cand)) ($([math]::Round((Get-Item $targetPack).Length/1MB, 1)) MB)!"
        break
    }
}

if (-not $packReady) {
    Write-Step "9/11" "Downloading Beatrix Shop Resource Pack ($exactPackName) from Google Drive..."
    $dlPack = Download-DriveFile -FileId $packId -OutFile $targetPack -Desc $exactPackName
    if ($dlPack) { $packReady = $true; Write-Success "Resource Pack downloaded successfully at: $targetPack" }
}

if ($packReady) {
    # Keep and sync ALL 3 variations so Minecraft matches ANY internal identifier
    $syncList = @(
        (Join-Path $ResourcePacksDir "beatrix_shop 1.9v1 (1).zip"),
        (Join-Path $ResourcePacksDir "beatrix_shop 1.9v1.zip"),
        (Join-Path $ResourcePacksDir "beatrix_shop.zip")
    )
    foreach ($syn in $syncList) {
        if (-not (Test-Path $syn)) { Copy-Item -Path $targetPack -Destination $syn -Force }
        Patch-BeatrixZip $syn
    }
    Write-Success "Synchronized & patched all 3 Resource Pack variations with native format 75!"
    ...
    for ($idx = 2; $idx -le $InstanceCount; $idx++) {
        ...
        foreach ($syn in $syncList) {
            Copy-Item -Path $syn -Destination (Join-Path $otherPackDir (Split-Path -Leaf $syn)) -Force
        }
    }
}
...
Write-Host " [STATUS] AutoSell: Keybind '[' | Spam /pay: Keybind '``' (phím ~) | No-Render: Keybind 'Z' | Auto-Join: donutsmp.net" -ForegroundColor Yellow
Write-Host "--------------------------------------------------------------------------" -ForegroundColor DarkGray
Write-Host ""
